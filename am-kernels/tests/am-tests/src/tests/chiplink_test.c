...
void chiplink_test()
{
    uintptr_t src_addr = 0xc0000000;
    uint32_t test_size = 1000;
    // 写测试
    uint8_t *byte_ptr = (uint8_t *)src_addr;
    // putstr("=== Byte Write ===\n");
    // for(int i = 0; i < test_size; ++i){
    //	byte_ptr[i] = i;
    // }
    uint16_t *half_ptr = (uint16_t *)src_addr;
    putstr("=== Half Write ===\n");
    ...
    // 读测试阶段
    byte_ptr = (uint8_t *)src_addr;
    half_ptr = (uint16_t *)src_addr;
    uint32_t *word_ptr = (uint32_t *)src_addr;
